document.addEventListener('DOMContentLoaded', function() {
        ...
        cart.forEach(item => {
            ...
            cartContainer.innerHTML += `
                <div class="cart-item flex flex-col sm:flex-row gap-6 p-6 bg-white/5 rounded-xl mb-4 hover:bg-white/10 transition-all duration-300 border border-white/10">
                    ...
                    <div class="flex-1">
                        <div class="text-white/60 text-xs uppercase tracking-wider mb-2 font-space">${item.category}</div>
                        <h3 class="font-orbitron font-bold text-lg mb-4">${item.name}</h3>
    ...
    window.decreaseQuantity = function(thisProduct) {
        const productName = thisProduct.closest('.cart-item').querySelector('h3').textContent;
